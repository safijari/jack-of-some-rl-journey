...
class SnakeModel(nn.Module):
    def __init__(self, input_shape, num_actions, num_hidden=512):
        ...
        self.policy = nn.Sequential(
            init_(nn.Linear(num_fc, num_hidden)), nn.ReLU(), init_(nn.Linear(num_hidden, num_actions))
        )
    ...
    def ppo_update(
        ...
        for _ in range(ppo_epochs):
            for state, action, old_log_probs, return_, advantage in ppo_iter(
                ...
                optimizer.zero_grad()
                loss = 1.0 * critic_loss + actor_loss - 0.0001 * entropy
                loss.backward()
                optimizer.step()
        ...
        return final_loss/final_loss_steps, factor_loss/final_loss_steps, fcritic_loss/final_loss_steps, fentropy_loss/final_loss_steps
...
def main(device="cuda"):
    ...
    env_fac = lambda: gym.make("snakenv-v0", gs=6, main_gs=6, num_fruits=1)
    num_envs = 32
    ...
    while True:
        ...
        loss, actor_loss, critic_loss, entropy_loss = model.ppo_update(50, 320, states, actions, log_probs, gae, advantage)
        wandb.log({'loss': loss, 'actor_loss': actor_loss, 'critic_loss': critic_loss, 'entropy_loss': entropy_loss}, step=idx)
